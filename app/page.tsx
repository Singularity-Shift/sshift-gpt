...
function GameOfLifeBackground() {
  const [grid, setGrid] = useState(() => createEmptyGrid(window.innerWidth, window.innerHeight))
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: -1 }}>
      <div style={{
        display: 'grid',
        gridTemplateColumns: `repeat(${Math.ceil(window.innerWidth / CELL_SIZE)}, ${CELL_SIZE}px)`,
        gridTemplateRows: `repeat(${Math.ceil(window.innerHeight / CELL_SIZE)}, ${CELL_SIZE}px)`,
        height: '100vh',
        width: '100vw',
      }}>
        {grid.map((row, i) =>
          row.map((cell, j) => (
            <div
              ...
              style={{
                width: CELL_SIZE,
                height: CELL_SIZE,
                backgroundColor: cell ? 'rgba(200, 200, 200, 0.2)' : 'transparent',
