...
const AppStatusControl = () => {
  ...
  const [pendingAction, setPendingAction] = useState<'stop' | 'resume' | null>(
    null
  );
  const { isAdmin, isReviewer, isAppRunning, setAppRunning } =
    useAppManagment();
  ...
  const handleConfirm = async () => {
    try {
      const tx = await client?.useABI(SubscriptionABI).trigger_app({
        type_arguments: [],
        arguments: [],
      });
      ...
      toast({
        ...
        description: (
          <div>
            App has been ${pendingAction === 'stop' ? 'stopped' : 'resumed'}{' '}
            successfull by transaction{' '}
            <Link href={`https://explorer.aptoslabs.com/txn/${tx?.hash}`} />
          </div>
        ),
        variant: 'default',
      });
      ...
      <div className="flex items-center space-x-4">
        <div
          className={`h-3 w-3 rounded-full ${
            isAppRunning ? 'bg-green-500' : 'bg-red-500'
          }`}
        ></div>
