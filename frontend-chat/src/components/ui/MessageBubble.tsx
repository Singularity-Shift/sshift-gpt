  ...
  ({ message, onCopy, onRegenerate, onEdit }) => {
    ...
    const safelyParseTwitterData = useMemo(() => {
      ...
      const parseTwitterMentionsFromText = (text: string): TwitterMention[] => {
        ...
        const mentionRegex =
          /(\d+)\.\s+\*\*([^*]+)\*\*\s*:\s*"([^"]+)"\s*(?:Likes:\s*(\d+)\s*\|\s*Quotes:\s*(\d+)\s*\|\s*Replies:\s*(\d+)\s*\|\s*Reposts:\s*(\d+)\s*\|\s*Views:\s*(\d+))?/g;
      ...
      try {
        ...
        if (contentObj.final_message?.content) {
          const extractedMentions = parseTwitterMentionsFromText(
